...
git -C "$CIPOOL_CHECKOUT_DIR" merge-base --is-ancestor "$source_sha" "refs/remotes/origin/$source_branch" || { echo "Authorized source SHA is not reachable from the declared branch" >&2; exit 67; }
...
cache_key="$CIPOOL_ROOT/cache-key"
python3 - "$CIPOOL_CHECKOUT_DIR" "$runtime" "$cache_key" <<'PY_CACHE'
...
if runtime in {"node22","playwright162"}:
    ...
    rels=["Cargo.lock","validator/java/pom.xml"]
    rels += sorted(str(p.relative_to(root)) for p in root.glob("packs/**/artifacts.lock.json"))
elif runtime=="machine":
    rels=["Cargo.lock","pnpm-lock.yaml","validator/java/pom.xml"]
else:
    rels=[]
h=hashlib.sha256()
for rel in rels:
    path=root/rel
    if not path.is_file():
        raise SystemExit(f"dependency fingerprint source missing: {rel}")
    data=path.read_bytes()
    h.update(rel.encode()+b"\0"+hashlib.sha256(data).digest())
out.write_text(h.hexdigest()+"\n",encoding="ascii")
out.chmod(0o600)
PY_CACHE
printf 'CIPOOL_EXECUTOR_PREPARED runtime=%s cache_key=%s\n' "$runtime" "$(<"$cache_key")"
